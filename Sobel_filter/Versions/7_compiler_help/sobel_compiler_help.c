...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	int  res1, res2;
	unsigned int p, i, j;
	register int res, size_i, posy1, posy2, posy3;
	...
	for (i=1; i<SIZE-1; i+=1 ) {
		...
		for (j=1; j<SIZE-1; j+=1) {		
			...
			*/
			...
			res1=0;
			/*i=-1*/
			/*changes: (a)posy , (b)obvious calculations(like: -1+1 = 0)*/
			posy1 = ((i - 1)<<12) + j;
			res1 += input[posy1 - 1] * horiz_operator[0][0];
			res1 += input[posy1 + 0] * horiz_operator[0][1];
			res1 += input[posy1 + 1] * horiz_operator[0][2];
			/*i=0*/
			posy2 =(i<<12) + j;
			res1 += input[posy2 - 1] * horiz_operator[1][0];
			res1 += input[posy2 + 0] * horiz_operator[1][1];
			res1 += input[posy2 + 1] * horiz_operator[1][2];
			/*i=1*/
			posy3 =((i+1)<<12)+ j;
			res1 += input[posy3 - 1] * horiz_operator[2][0];
			res1 += input[posy3 + 0] * horiz_operator[2][1];
			res1 += input[posy3 + 1] * horiz_operator[2][2];
	
			res2=0;
			/*i=-1*/
			res2 += input[posy1 - 1] * vert_operator[0][0];
			res2 += input[posy1 + 0] * vert_operator[0][1];
			res2 += input[posy1 + 1] * vert_operator[0][2];
			/*i=0*/
			res2 += input[posy2 - 1] * vert_operator[1][0];
			res2 += input[posy2 + 0] * vert_operator[1][1];
			res2 += input[posy2 + 1] * vert_operator[1][2];
			/*i=1*/
			res2 += input[posy3 - 1] * vert_operator[2][0];
			res2 += input[posy3 + 0] * vert_operator[2][1];
			res2 += input[posy3 + 1] * vert_operator[2][2];
...
/*			if (res > 255)
				...
				output[size_i + j] = (unsigned char)res;
*/
...
/*			The Look Up Table gives worst results			

			if (p > 65536)
				output[size_i + j] = 255;
			else{
				switch (p) {
					case 400:	res = 20; break;
					//case 0:		res = 0; break;
					//case 16:	res = 4; break;
					//case 64:	res = 8; break;
					//case 144:	res = 12; break;
					//case 576:	res = 24; break;
					default:	res = (int)sqrt(p);
				}
				output[size_i + j] = (unsigned char)(res);
			}
*/			
	...
	for (i=1; i<SIZE-1; i++) {
		size_i = (i<<12);
		for ( j=1; j<SIZE-1; j++ ) {
			t = (output[size_i+j] - golden[size_i+j]);
			PSNR += t*t;
		}
	}
