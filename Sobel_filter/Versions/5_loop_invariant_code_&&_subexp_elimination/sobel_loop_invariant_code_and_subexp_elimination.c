...
{
	double PSNR = 0, t;
	int i, j, res1, res2, n_size, size_i, posy;
	unsigned int p;
	int res;
	...
	n_size = SIZE-1;
	for (i=1; i<n_size; i+=1 ) {
		size_i = i*SIZE;
		for (j=1; j<n_size; j+=1) {		
			...
			*/
			...
			posy = (i - 1)*SIZE + j;
			res1 += input[posy - 1] * horiz_operator[0][0];
			res1 += input[posy + 0] * horiz_operator[0][1];
			res1 += input[posy + 1] * horiz_operator[0][2];
			/*i=0*/
			posy = (i)*SIZE + j;
			res1 += input[posy - 1] * horiz_operator[1][0];
			res1 += input[posy + 0] * horiz_operator[1][1];
			res1 += input[posy + 1] * horiz_operator[1][2];
			/*i=1*/
			posy = (i + 1)*SIZE + j;
			res1 += input[posy - 1] * horiz_operator[2][0];
			res1 += input[posy + 0] * horiz_operator[2][1];
			res1 += input[posy + 1] * horiz_operator[2][2];
	
			res2=0;
			/*i=-1*/
			posy = (i - 1)*SIZE + j;
			res2 += input[posy - 1] * vert_operator[0][0];
			res2 += input[posy + 0] * vert_operator[0][1];
			res2 += input[posy + 1] * vert_operator[0][2];
			/*i=0*/
			posy = (i)*SIZE + j;
			res2 += input[posy - 1] * vert_operator[1][0];
			res2 += input[posy + 0] * vert_operator[1][1];
			res2 += input[posy + 1] * vert_operator[1][2];
			/*i=1*/
			posy = (i + 1)*SIZE + j;
			res2 += input[posy - 1] * vert_operator[2][0];
			res2 += input[posy + 0] * vert_operator[2][1];
			res2 += input[posy + 1] * vert_operator[2][2];
	...
	for (i=1; i<n_size; i++) {
		size_i = i*SIZE;
		for ( j=1; j<n_size; j++ ) {
			t = pow((output[size_i+j] - golden[size_i+j]),2);
			PSNR += t;
		}
	}
