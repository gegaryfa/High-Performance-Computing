...
////////////////////////////////////////////////////////////////////////////////
#define ROW_TILE_W 				64
#define ROW_TILE_H				16					//MUST BE ALIGNED TO BLOCK_H
...
////////////////////////////////////////////////////////////////////////////////
#define COLUMN_TILE_W 16							//MUST BE ALIGNED TO BLOCK_H
#define COLUMN_TILE_H 64
							...
							int imgWidth) 
{
	...
	if ( (threadIdx.y  <  filterRad))
		cache[threadIdx.y*COLUMN_TILE_W + threadIdx.x] =d_Src[ iy *(2 * filterRad + imgWidth) + ix + filterRad];
	
	cache[(threadIdx.y+filterRad)*COLUMN_TILE_W + threadIdx.x] =d_Src[idx];

	if ( (threadIdx.y  >=  blockDim.y- filterRad ))
		cache[(threadIdx.y + 2*filterRad)*COLUMN_TILE_W + threadIdx.x] =d_Src[(iy + 2*filterRad) *(2 * filterRad + imgWidth) + ix + filterRad];
	...
	for (k = -filterRad; k <= filterRad; k++) {
		...
		sum += cache[(d + filterRad)*COLUMN_TILE_W + threadIdx.x] * const_Filter[filterRad - k];
...
sum1 = 0;
	...
	//cudaFuncSetCacheConfig(convolutionRowGPU, cudaFuncCachePreferL1);
	//cudaFuncSetCacheConfig(convolutionColumnGPU, cudaFuncCachePreferL1);
	cudaEventRecord(start);
