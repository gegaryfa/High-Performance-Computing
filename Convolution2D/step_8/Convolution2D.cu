...
#define accuracy  	0.000000000000000005	 
...
__global__ void convolutionRowGPU(real *d_Dst,
	...
	int ix = blockIdx.x * blockDim.x + threadIdx.x + filterR;	//col
	int iy = blockIdx.y * blockDim.y + threadIdx.y + filterR;	//row
	
	int grid_width = gridDim.x * blockDim.x + (2*filterR); 
...
sum1 = 0;
	...
	cudaEventRecord(start);
	convolutionRowGPU<<<grid_size,block_size>>>(d_padded_Buffer, d_padded_Input, d_Filter, filter_radius); // convolution kata grammes
	CudaCheckError();
    convolutionColumnGPU<<<grid_size,block_size>>>(d_padded_OutputGPU, d_padded_Buffer, d_Filter, filter_radius); // convolution kata sthles
	CudaCheckError();
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
