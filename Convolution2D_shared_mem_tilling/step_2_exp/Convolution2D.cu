...
#define FILTER_RAD	16
#define TILE_W		16
#define TILE_H		16

__constant__ float const_Filter[2 * FILTER_RAD + 1];
 ...
 *
*/
__global__ void convolutionRowGPU(float *d_Dst, float *d_Src, int imgSize)
{
	
	__shared__ float cache[TILE_H][TILE_W + 2 * FILTER_RAD];
	int ix = threadIdx.x + blockDim.y * blockIdx.x;
	int iy = threadIdx.y + blockDim.y * blockIdx.y + FILTER_RAD;
	
	int imgWidth = imgSize + 2 * FILTER_RAD;

	if ((ix < FILTER_RAD) || (ix >= imgSize+FILTER_RAD))
		cache[threadIdx.y][threadIdx.x] = 0.f;
	else
		cache[threadIdx.y][threadIdx.x] = d_Src[iy * imgWidth + ix];
	...
	if ((threadIdx.x >= FILTER_RAD) && (threadIdx.x < TILE_W + FILTER_RAD)) {
		for (k = -FILTER_RAD; k <= FILTER_RAD; k++) {
			int d = threadIdx.x + k;

			sum += cache[threadIdx.y][d] * const_Filter[FILTER_RAD - k];
		}
		d_Dst[iy * imgWidth + ix] = sum;
	}
 ...
 *
*/
__global__ void convolutionColumnGPU (float *d_Dst, float *d_Src, int imgSize)
{
	__shared__ float cache[TILE_H + 2 * FILTER_RAD][TILE_W];
	int ix = threadIdx.x + blockDim.x * blockIdx.x + FILTER_RAD;
	int iy = threadIdx.y + blockDim.x * blockIdx.y;
	
	int imgWidth = imgSize + 2 * FILTER_RAD;

	if ((iy < FILTER_RAD) || (iy >= imgSize + FILTER_RAD))
		cache[threadIdx.y][threadIdx.x] = 0.f;
	else
		cache[threadIdx.y][threadIdx.x] = d_Src[iy * imgWidth + ix];
	...
	if ((threadIdx.y >= FILTER_RAD) && (threadIdx.y < TILE_H + FILTER_RAD)) {
		for (k = -FILTER_RAD; k <= FILTER_RAD; k++) {
			int d = threadIdx.y + k;

			sum += cache[d][threadIdx.x] * const_Filter[FILTER_RAD - k];
		}
		d_Dst[iy * imgWidth + ix] = sum;
	}
}
...
int main(int argc, char **argv) {
	...
	*/
	...
	int sizeof_filter = FILTER_LENGTH * sizeof(real);
...
sum1 = 0;
	...
	/* create thread blocks and block grids for row convolution */
	dim3 row_block_size;
	row_block_size.x = TILE_W + 2 * filter_radius;
	row_block_size.y = TILE_H;

	dim3 row_grid_size;
	row_grid_size.x = imageW/TILE_W;
	row_grid_size.y = imageH/TILE_H;

	/* create thread blocks and block grids for col convolution */
	dim3 col_block_size;
	col_block_size.x = TILE_W;
	col_block_size.y = TILE_H + 2 * filter_radius;

	dim3 col_grid_size;
	col_grid_size.x = imageW/TILE_W;
	col_grid_size.y = imageH/TILE_H;


	cudaEventRecord(start);	

	convolutionRowGPU<<<row_grid_size,row_block_size>>>(d_padded_Buffer, d_padded_Input, imageW);
	CudaCheckError();

    convolutionColumnGPU<<<col_grid_size,col_block_size>>>(d_padded_OutputGPU, d_padded_Buffer, imageH);
	CudaCheckError();
