...
/*Unroll try works!!*/
	sum =cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -16) + filterRad] * const_Filter[0]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -15) + filterRad] * const_Filter[1]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -14) + filterRad] * const_Filter[2]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -13) + filterRad] * const_Filter[3]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -12) + filterRad] * const_Filter[4]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -11) + filterRad] * const_Filter[5]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -10) + filterRad] * const_Filter[6]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -9) + filterRad] * const_Filter[7]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -8) + filterRad] * const_Filter[8]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -7) + filterRad] * const_Filter[9]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -6) + filterRad] * const_Filter[10]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -5) + filterRad] * const_Filter[11]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -4) + filterRad] * const_Filter[12]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -3) + filterRad] * const_Filter[13]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -2) + filterRad] * const_Filter[14]+
		cache[threadIdx.y*(ROW_TILE_W + 2*filterRad) + (threadIdx.x -1) + filterRad] * const_Filter[15];

	for (k = 0; k <= filterRad; k++) {
		int d = threadIdx.x + k;
...
__global__ void convolutionColumnGPU(real *d_Dst,
							...
							int imgWidth) 
{
	__shared__ real cache[(COLUMN_TILE_H + 2*filterRad)*COLUMN_TILE_W];	//[COLUMN_TILE_H + 2*filterRad][COLUMN_TILE_H]
...
sum1 = 0;
	...
	/* imageH x imageW threads per block */
	dim3 col_block_size;
	col_block_size.x = BLOCK_W;
	col_block_size.y = BLOCK_H;

	/* n x n grid*/
	dim3 col_grid_size;
	col_grid_size.x = imageW/BLOCK_W;
	col_grid_size.y = imageH/BLOCK_H;
