...
/* Search for all the elements that contain the source node */
	...
	if (Src.sourcenode != 0) {
		#pragma omp parallel for private(i, j, k, cor, xc, vertices) //schedule(dynamic)
...
/* Simulation */
	//#pragma omp parallel for private(i, j, k, Me, Ce, v, Ke, cor, verticesonbnd, bv, vertices, alpha, kk, ii, jj, xc, Mexv, Cexv)   //CHECK-->Sometimes different results?
	for (i = 0; i < ARCHelems; i++) {
		for (j = 0; j < 12; j++) {
			Me[j] = 0.0;
			Ce[j] = 0.0;
...
double m[][12], v[];
{
  ...
  double u[12];
	...
	////////#pragma omp parallel for private(i)	//LOSE ACCURACY
	for (i = 0; i < 12; i++) 
		v[i] = u[i];
                ...
                double (*v)[3], double (*w)[3])
{
	...
	#pragma omp parallel for private(i,j) //schedule(dynamic, 512)  //CHECK--> a little worst with dynamic
	for (i = 0; i < nodes; i++) {
		for (j = 0; j < numthreads; j++) {
			if (w2[j][i]) {
				w[i][0] += w1[j][i].first;
...
/* Auxiliary vector */
  ...
  if (w2 == (int **) NULL) {
	...
	//#pragma omp parallel for private(i) // CHECK --> GOOD
	for (i = 0; i < numthreads; i++) {
	...
	#pragma omp parallel for private(i,j) //CHECK --> DIFFERENT RESULTS //!!!
	for (j = 0; j < numthreads; j++) {
		for (i = 0; i < ARCHnodes; i++) {
			w2[j][i] = 0;
		}
  ...
  /* Initializations */
	#pragma omp parallel for private(i,j)		//---> LOOSE ACCURACY
	for (i = 0; i < ARCHnodes; i++) {
		nodekind[i] = 0;
		for (j = 0; j < 3; j++) {
			M[i][j] = 0.0;
	...
	#pragma omp parallel for private(i, j, k) //---- WORST TIME
	for (i = 0; i < ARCHmatrixlen; i++) {
		for (j = 0; j < 3; j++) {
			for (k = 0; k < 3; k++) {
				K[i][j][k] = 0.0;
