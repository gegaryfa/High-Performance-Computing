...
*/
	...
	///////me grid size ==x * block_size, me megalo x  kalutera!!		//mexri twra to 212 exei doulepsei kalutera gia to 1!!
	PGM_IMG h_result;
	h_result.w = img_in.w;
	h_result.h = img_in.h;
...
memset(h_hist, 0, 256 * sizeof(int));
	...
	/*ALL THE CALLS TOGETHER TO OVERLAP MEMORY TRANSFERS*/
	cudaEventRecord(start);
	CudaSafeCall( cudaMemcpyAsync(d_hist, h_hist, hist_size_B, 	cudaMemcpyHostToDevice) );
	CudaSafeCall( cudaMemcpyAsync(d_img, img_in.img, img_size_B, cudaMemcpyHostToDevice) );
	histogramGPU_2<<<1024,256>>>(d_hist, d_img, img_size); //Histogram kernel
	CudaSafeCall( cudaMemcpyAsync(h_hist, d_hist, hist_size_B, 	cudaMemcpyDeviceToHost) );
	create_lut(h_hist, h_result.w * h_result.h, 256, &h_lut);
	CudaSafeCall( cudaMemcpyAsync(d_lut, h_lut, lut_size_B, cudaMemcpyHostToDevice) );
	histogram_equalizationGPU8<<<grid_size, block_size>>>(d_result, d_img, d_lut, img_size);
	CudaSafeCall( cudaMemcpyAsync(h_result.img, d_result, img_size_B, cudaMemcpyDeviceToHost) );
	CudaCheckError();
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
...
// Choose method
/*naive - WORKS FINE. TIME FOR 6400x6400 image =~ 52ms*/
/*
	cudaEventRecord(start);
	histogram_equalizationGPU1<<<15, 1024>>>(d_result, d_img, d_lut, img_size);
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
...
/*naive2 - WORKS FINE. TIME FOR 6400x6400 image =~ 76ms [1x256]*/
/*
	cudaEventRecord(start);
	histogram_equalizationGPU2<<<1, 256>>>(d_result, d_img, d_lut, img_size);
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&GPU_time_calc_eq, start, stop);
*/
...
/*Shared memory - WORKS FINE. TIME FOR 6400x6400 image =~ 50ms [1x1024])*/
/*
	cudaEventRecord(start);
...
/*Constant memory - WORKS FINE. TIME FOR 6400x6400 image =~ 67ms [1x1024])*/
/*
	cudaEventRecord(start);
...
/*Shared memory - WORKS FINE. TIME FOR 6400x6400 image =~ 35ms [1x1024])*/
/*
	block_size.x = 1024;
...
/*Shared memory - WORKS FINE. TIME FOR 6400x6400 image =~ 34ms [1x1024])*/
/*
	block_size.x = 1024;
...
/*Shared memory - WORKS FINE. TIME FOR 6400x6400 image =~ 32ms [1x1024])*/
/*
	block_size.x = 1024;
...
/*Shared memory - WORKS FINE on MARS2 BUT NOT ON ZEUS2. TIME FOR 6400x6400 image =~ 33ms [1x1024])*/
/*
	block_size.x = 1024;
	grid_size.x = (img_size/16)/1024;
	if ( (img_size/16) % block_size.x)
		grid_size.x += 1;
	cudaEventRecord(start);
	histogram_equalizationGPU8<<<grid_size, img_size>>>(d_result, d_img, d_lut, img_size);
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&GPU_time_calc_eq, start, stop);
*/

/*Shared memory - WORKS FINE. TIME FOR 6400x6400 image =~ 34ms [1x768])*/
/*
	block_size.x = 768;
...
*/
	...
	printf(ANSI_COLOR_GREEN"...GPU processing finished\n\n"ANSI_COLOR_WHITE);
...
*/
	printf(ANSI_COLOR_GREEN"\n==============TIMES==============");
	printf ("\nTotal PARALLEL/OPTIMIZED code time %.7g ms\n"ANSI_COLOR_WHITE, GPU_time);
